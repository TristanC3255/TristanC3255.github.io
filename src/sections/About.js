// Data for the About section (kept near top for clarity)
const PROFILE_OVERVIEW = [
  { label: 'Location', value: 'New York, NY' },
  { label: 'School', value: 'CCNY' },
...
function About() {
  // About section - left column contains bio, right column contains small cards
  return (
    <section id="about">
      {/* Section title */}
      <h2>About Me</h2>

      {/* Grid layout: main content and right-hand cards */}
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem', alignItems: 'start' }}>
        ...
        {/* Right: small informational cards stacked vertically */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>

          {/* Overview card: short profile facts */}
          <div style={{
            background: 'var(--card)',
            ...
            <h3 style={{ margin: '0 0 1rem 0', color: 'var(--accent)', fontSize: '1.1rem' }}>Overview</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {PROFILE_OVERVIEW.map((item) => (
                <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--muted-text)', fontSize: '0.9rem' }}>{item.label}</span>
          ...
          {/* Resume card: download link and small note */}
          <div style={{
            background: 'var(--card)',
            border: '1px solid var(--border)',
            borderRadius: '12px',
            ...
            <h3 style={{ margin: '0 0 1rem 0', color: 'var(--accent)', fontSize: '1.1rem' }}>Resume</h3>
            ...
            <p style={{ margin: '0.75rem 0 0 0', fontSize: '0.8rem', color: 'var(--muted-text)' }}>{RESUME.note}</p>
          </div>

        </div>
      </div>

      {/* Mobile responsive layout (same as before) */}
      <style jsx>{`
